...
class Creature {
  ...
  // update(param,db,allCreatures,allItems, walls = []){
	...
	walking(){
		...
		if(this.walk >= game.time.getTime() 
		...
		let phantomPos = this.handleWalking([...this.position])
		// let phantomPos = this.handleWalking([this.position[0],this.position[1],this.position[2]])
		
		// CHEKCKING IF POSITION IS AVAILBLE
		
		if(!this.isPosAvalible(phantomPos)){
			this.text = "There's no way."
			return null
		}

		phantomPos = this.handleSpecialGrids(phantomPos)
